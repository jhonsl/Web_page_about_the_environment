...
var scroll = new SmoothScroll('a[href*="#"]', {customEasing: function (time) {return time < 0.5 ? 2 * time * time : -1 + (4 - 2 * time) * time;}});
document.addEventListener("DOMContentLoaded", function(){var buttonUp = document.querySelector('a[id="top_button"]');var easings = {easeInOutQuint(t) {return t < 0.5 ? 16 * t * t * t * t * t : 1 + 16 * (--t) * t * t * t * t;}};function isVisibled() {var heightHide = parseInt(buttonUp.getAttribute('height-hide')) || 100;if (document.body.scrollTop > heightHide || document.documentElement.scrollTop > heightHide) {buttonUp.classList.remove('simplescrollup__button--hide');buttonUp.classList.add('simplescrollup__button--show');} else {buttonUp.classList.remove('simplescrollup__button--show');buttonUp.classList.add('simplescrollup__button--hide');}}window.addEventListener('scroll', isVisibled);});